  ...
  useEffect(() => {
    ...
    unsubscribeDigest(digestId, token)
      ...
      .catch((e) => {
        if (cancelled) return
        const msg =
          e instanceof ApiError ? e.message : "Couldn't reach the server. Try again from the email."
  ...
  return (
    <main className="min-h-screen">
      <div className="mx-auto flex max-w-md flex-col items-center px-6 py-20 text-center">
        ...
        <Link href="/login" className="mt-10 text-sm text-muted-foreground hover:text-primary">
...
function SuccessCard({ title, body, at }: { title: string; body?: string; at: string }) {
  return (
    <div className="rounded-lg border border-border bg-canvas p-6">
      ...
      <p className="mt-3 text-xs text-muted-foreground">Confirmed at {formatTimestamp(at)}.</p>
    </div>
  )
}
