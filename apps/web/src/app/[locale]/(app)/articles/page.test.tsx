 ...
 *
 * i18n (Task #32): the `TIER_HEADINGS` export was removed — labels now
 * come from `useTranslations('Tiers')` in the page. Tests below mirror
 * the page's JSX using the literal English strings from `messages/en.json`
 * so the assertions still verify the same tier order / placeholder text.
 */

import { describe, it, expect, beforeEach, vi } from 'vitest'
import { render, screen } from '@testing-library/react'
import { IntlWrapper } from '@/test-utils/intl-helper'
import { bucketByTier, isTier, TIER_ORDER } from '@/app/[locale]/(app)/articles/buckets'
...
describe('/articles page — view rendering', () => {
  ...
  // the pure tree-assembly it produces by mirroring its JSX here. The mirror
  // uses literal English strings sourced from `messages/en.json` so this
  // test catches drift between the message catalog and the JSX shape.
  const TIER_HEADINGS = {
    must_read: 'Must Read',
  ...
  it('renders only must_read articles when tier filter is applied (flat view)', () => {
    ...
    render(
      <IntlWrapper>
        <>{renderView({ items: must_read, grouped: false })}</>
      </IntlWrapper>
    )
