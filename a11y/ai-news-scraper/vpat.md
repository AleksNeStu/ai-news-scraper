...
| 2.4.1 | Bypass Blocks | A | Does Not Support | Skip-link pending (finding #7). |
...
| 2.4.7 | Focus Visible | AA | Partially Supports | Default browser indicator weak on dark theme; `:focus-visible` rule pending (finding #9). |
...
| 2.5.8 | Target Size (Minimum) | AA (2.2) | Partially Supports | Nav links reviewed; smaller interactive elements may need padding; automated axe scan pending (finding #11). |
...
| 3.3.1 | Error Identification | A | Partially Supports | Per-field errors surfaced (Task #36); `role="alert"` / `aria-live` placement pending manual verification (finding #12). |
...
| 3.3.7 | Redundant Entry | A (2.2) | **Not Evaluated** | Pre-fill of returning user's email pending (finding #13). |
...
| 4.1.2 | Name, Role, Value | A | Partially Supports | Most controls correctly labelled; NotificationBell pending (findings #6, #14). |
