...
| 8 | ~~MAJOR~~ | 2.4.3 Focus Order | `apps/web/src/components/auth/LogoutButton.tsx` | **Closed + verified 2026-07-11** — Frontend commit `96db4f6` clears focus on pending state via `button.blur()` and preserves the "Logging out…" announcement via `aria-busy`; the success path relocates focus to the post-logout `<h1>` via the Next.js route swap, and the server-error path leaves focus on the re-enabled trigger (no focus trap). Practical UX is correct; the row above now describes the actual mechanism rather than overstating the component's role. | Source re-review |
...
| 11 | ~~MINOR~~ | 2.5.8 Target Size (Minimum) | WCAG 2.2 — 24×24px | **Closed + runtime-verified 2026-07-12** — Original closure (commit `6edf2e0`) added `min-h-6` to `NavLink` in `AppHeader.tsx:92`, giving every header nav link at least a 24px tall bounding box on wrap. The 2026-07-12 verification-polish phase added an automated `e2e/a11y-extra.spec.ts` that walks all 13 audit-report routes, measures every interactive element's bounding box, and flags anything below 24×24 px (excluding the WCAG 2.5.8 "equivalent" / inline-text-link exception per W3C understanding doc). One remaining link failed: the "Back to AI News Search →" link on `/en/unsubscribe` was 160×21 px. Systemic fix in commit `f28a55a`: applied the same `inline-flex min-h-6 items-center px-3` pattern from `AppHeader.tsx` to the unsubscribe back link. Re-run after the fix: 0 violations across 13 routes. | Source re-review + axe + runtime Playwright |
